...
class Router extends EventEmitter {
  constructor(config) {
    ...
    this.protocols.ws.on('getProtocolStatus', () => {
      const protocolStatus = {};
      Object.keys(this.protocols).forEach((protocol) => {
        protocolStatus[protocol] = this.protocols[protocol].status;
      });
      // TODO(jwetzell): only send protocol status to the socket that requested it
      this.protocols.ws.sendToWebUISockets('protocolStatus', protocolStatus);
    });
