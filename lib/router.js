          ...
          if (trigger.shouldFire(msg)) {
            ...
            trigger.actions.forEach((action, actionIndex) => {
              if (!action.enabled) {
                ...
                this.protocols.ws.sendToWebUISockets('action', {
                  action: action.toJSON(),
                  path: `${triggerPath}/actions/${actionIndex}`,
                  fired: false,
                });
                return;
              }
              ...
              try {
                action.on('transform', (transform, transformIndex, fired) => {
                  this.emit(
                    'transform',
                    transform.toJSON(),
                    `${triggerPath}/actions/${actionIndex}/transforms/${transformIndex}`,
                    fired
                  );
                  this.protocols.ws.sendToWebUISockets('transform', {
                    transform: transform.toJSON(),
                    path: `${triggerPath}/actions/${actionIndex}/transforms/${transformIndex}`,
                    fired,
                  });
                });
