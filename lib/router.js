...
class Router extends EventEmitter {
  ...
  stop() {
    ...
    logger.info('app: waiting for all protocols to say they have stopped');
    const protocolKeys = Object.keys(this.protocols);
    protocolKeys.forEach((protocol) => {
      this.protocols[protocol].on('stopped', () => {
        protocolsClosed += 1;
        logger.trace(`app: protocol ${protocol} has stopped`);
        if (protocolsClosed === protocolKeys.length) {
          logger.info('app: all protocols have stopped');
          this.emit('stopped');
        }
      });
    ...
    setTimeout(() => {
      logger.warn('app: protocols have taken a while to stop');
      this.emit('stopped');
    }, 5000);
  }
