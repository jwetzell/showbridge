...
class TCPProtocol extends EventEmitter {
  ...
  reload(params) {
    ...
    this.server.listen(
      ...
      () => {
        logger.debug(
          `tcp: server setup on port ${(this.server.address() as AddressInfo).address}:${(this.server.address() as AddressInfo).port}`
        );
        this.emit('started');
      }
    );
  ...
  send(msg: Buffer, port: number, host: string, slipEncode: boolean) {
    ...
    if (this.sockets[host][port] === undefined) {
      this.sockets[host][port] = new Socket();
  ...
  #bufferToMessage(buffer: Buffer, host: string, port: number) {
    let message;
