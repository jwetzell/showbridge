...
class Config {
  ...
  loadTriggers() {
    Object.keys(Types.MessageTypeClassMap).forEach((messageType) => {
      if (this.config[messageType]) {
        ...
        this[messageType].triggers = this[messageType]?.triggers
          ?.filter((trigger) => has(Types.TriggerTypeClassMap, trigger.type))
          .map((trigger) => new Types.TriggerTypeClassMap[trigger.type](trigger));
      }
    });
  }
