...
describe('Config', () => {
  ...
  test('cloud room migration', () => {
    ...
    assert.deepStrictEqual(config.protocols.cloud.params.rooms, ['test']);
  });
  ...
  test('toJSON', () => {
    ...
    assert.strictEqual(toJSON.handlers.http.triggers[0].comment, undefined);
    assert.strictEqual(toJSON.handlers.http.triggers[0].type, 'any');
    assert.strictEqual(toJSON.handlers.osc.triggers[0], undefined);
    assert.strictEqual(toJSON.handlers.ws.triggers[0].actions[0].type, 'log');
    assert.strictEqual(toJSON.handlers.ws.triggers[0].actions[0].comment, undefined);
    assert.strictEqual(toJSON.handlers.ws.triggers[0].actions[0].transforms.length, 0);
